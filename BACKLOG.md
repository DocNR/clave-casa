...
- [ ] **web: `static/sdk/clave-connect.js` v0 + npm mirror — RESCOPED 2026-09-02 to a thin connect/handoff shim** (decision after reading conduit-mono: the first partner already ships a hardened NIP-46 client, so extracting POWR's ~300-line client would duplicate what they have). v0 is: (1) build the `nostrconnect://` URI from a caller-supplied client pubkey + secret + metadata (`name`/`url`/`image`) with `wss://relay.powr.build` merged into the relay set; (2) wrap it as the `https://clave.casa/connect/?uri=` Universal Link and open it correctly per platform (mobile Safari: real `<a target="_self">`; desktop: QR via the `/connect` page); (3) the **iOS return-path contract** as code + docs — on `visibilitychange`/foreground, reconnect and send the `get_public_key` resume probe (auto-allowed for paired clients, never prompts — `LightSigner.swift:397`; confirms sessions that acked at least once, cannot bootstrap a lost ack); ack-timeout = retry-not-error; same-URI re-fire of a still-pending attempt (retain the secret inside the re-ack window; re-mint only after denial/expiry/establishment — dissolves deferred-deep-link + expiry); (4) `canOpenURL("clave://")`-style install detection is native-only, so the web shim documents the SKOverlay/Smart-Banner limits instead. The partner keeps their own NIP-46 transport (Conduit: nostr-tools `BunkerSigner` behind NDK) — the shim hands them a URI + a return-path hook, nothing more. Still: versioned immutable paths via `_headers`, published SRI hashes, connect origin hardcoded to clave.casa; fetch-kind:0-before-offering-profile; tolerate profile-less pubkeys; never hardcode proxy.clave.casa. The fuller "vendored NIP-46 client" remains an option for partners who have none (POWR's core is the source) — separate, later item.
- [x] **web: `static/brand/` "Connect with Clave" button — DONE 2026-09-02** (`7bc368d` + relabel; signed off by Daniel, who chose the label "Connect with Clave" over "Sign in with Clave" for the button — the *initiative* keeps its name. Neutral `--light`/`--dark` variants are the recommended default, `--brand` for a page's one primary action). OAuth-idiom button: 44px (override to 40px beside Google), 12px radius, current mark clipped to a 20px circle, 15px/600 system-stack label, site tokens throughout. Ships as a self-contained `connect-with-clave.css` (classes `.clave-connect*`, mark embedded), three 200×44 SVGs, and a partner README. Still to do in this area: wire the click via the rescoped `clave-connect.js`; update `docs/integrations.md` §Brand to point at `static/brand/` instead of "there isn't an official button yet".
...
- [x] **iOS: "Universal Link opens but no ApprovalSheet" — was TWO bugs, both FIXED 2026-09-02.** (An earlier note here said "not a delivery bug / AASA discarded" — wrong; the sim can only fire the scheme form.) **Bug 1, delivery:** on a real iPhone the `https` link produced zero `[Deeplink] received` lines — iOS delivers Universal Links as an NSUserActivity (`UIActivityContinuationAction`), and `ClaveApp` only had `.onOpenURL`, so the link was dropped before the router. Fix: `.onContinueUserActivity(NSUserActivityTypeBrowsingWeb)` feeding the same router. This is why the shipped UL has been dead since May. **Bug 2, presentation:** UIKit drops a `.sheet(item:)` set during a transition/competing modal; fix: consume + `asyncAfter(+0.6s)` + drain in `onAppear`. **Device-verified (real link, cold launch, existing user):** ApprovalSheet presented; partner-sim got ack + promptless probe (1.2s) + verified kind:1 sig (7.7s). **New-user path device-verified too (TF 1.1 (104), real link, zero accounts, cold launch):** banner → Generate → replay → sheet → approve; partner-sim got ack from a brand-new signer + promptless probe (1.6s) + verified sig (13.5s). **Lock-screen push leg also PASSED on 104** (Low-trust pairing, phone locked, sign_event arrived as a lock-screen banner, long-press Approve, verified sig 18.5s). **All on-device gates for this item are closed.** Remaining week-1 gates are the separate scratch-partner-app tests (SKOverlay + canOpenURL) and the Smart-Banner `app-argument` check.
