...
- [ ] **iOS: DeeplinkRouter stash-and-replay** — `.stashForOnboarding` replaces the 0-account `.ignore` (`Shared/DeeplinkRouter.swift:56`); first `clave://connect?uri=` handler for the reserved scheme; stash persisted with TTL (Phase 1: 10 min), single-slot last-writer-wins, promoted into `pendingNostrconnectURI` when `currentAccount` lands so the existing HomeView → ApprovalSheet replay fires (replay pairs the promoting account; never re-enters the account picker). `createdDuringFlow` lives on the promoted payload, set by `addAccount` iff the source was onboarding Generate — it dies with the replay. Onboarding step-1 caller banner ("_Partner_ wants to connect — create or import your key to continue"), same domain-first/unverified-metadata rules as ApprovalSheet; banner clears (or swaps to retry copy) on stash expiry. NOTE: onboarding step 2 is unreachable dead code — don't build on it.
...
- [ ] **web: `static/sdk/clave-connect.js` v0 + npm mirror** — extracted from POWR's vendored ~300-line NIP-46 client (rust-nostr ≤0.44.2's echoed-secret bug breaks its *bunker://* connect path; nostrconnect reportedly works — see clave `docs/nip46-compatibility.md`); versioned immutable paths via `_headers`, published SRI hashes, connect origin hardcoded to clave.casa. SDK contract: persist client keypair across retries, re-mint fresh secret per foreground (dissolves deferred-deep-link + expiry), `get_public_key` resume probe on foreground (auto-allowed for paired clients, never prompts — `LightSigner.swift:397`), ack-timeout = retry not error, fetch-kind:0-before-offering-profile, tolerate profile-less pubkeys, never hardcode proxy.clave.casa.
...
- [ ] **Week-1 empirical gates** — tooling shipped 2026-09-02 at `clave/scripts/signin-poc/` (relay ephemeral probe + partner simulator; the simulator proves handshake + promptless resume probe + lock-screen signing against the shipped build with zero iOS diffs — run it first). Remaining device gates: does relay.powr.build store kind:24133 (ephemeral range)?; on-device stash→generate/import→replay incl. partner-killed-during-install + re-fire re-ack timing vs the socket grace window; SKOverlay + canOpenURL from a scratch partner app; Smart-Banner OPEN `app-argument` on the real page.
...
- [ ] **iOS: idempotent connect re-ack window** — identical client pubkey + secret within ~10 min of successful pairing → silent duplicate ack (already-paired-only). Triggers only on same-URI redelivery (the /connect page's stashed-URI "Open Clave" re-fire, OS link redelivery, non-SDK clients re-sending connect); SDK clients never hit it — fresh secret per retry, their recovery is the resume probe. Write up as the NIP-46 mobile erratum.
