...
- [ ] **web: `static/brand/` "Sign in with Clave" button** — promised in integrations.md since May. Spec'd 2026-09-02: an OAuth-style button in the "Sign in with Google / Apple" idiom — rounded rect, Clave mark left, "Sign in with Clave" label — built from the current logo (`static/clave-icon.png`) and the site's own design tokens; light + dark variants, hover/pressed/focus states, accessible contrast, fixed-height so partners can line it up beside Google/Apple buttons; shipped as SVG/PNG assets + a drop-in HTML/CSS snippet that the rescoped `clave-connect.js` shim wires up.
