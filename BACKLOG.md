...
- [ ] **iOS: Universal Link entry does not surface approval on the shipped build (NEW, found during the gate; top on-device gate for Phase 1)** — tapping `clave.casa/connect/?uri=` opens Clave but shows no approval sheet + empty inbox (cold AND warm); the same inner `nostrconnect://` scanned via the in-app Connect scanner works end-to-end. Pure `DeeplinkRouter`+`NostrConnectParser` route the exact link correctly when called directly, so the defect is in on-device Universal-Link delivery into the routing chain (`onOpenURL`→NotificationCenter→observer→`pendingNostrconnectURI`→HomeView `.onChange`), not the parser/router. Root-cause candidates: AASA association for `/connect` on deployed clave.casa vs. the delivery chain. Repair, not a fork; the Phase-1 stash-and-replay shares this chain, so root-cause it there (device `[Deeplink] received` os_log, or `xcrun simctl openurl` repro).
