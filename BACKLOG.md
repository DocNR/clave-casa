...
- [x] **iOS: "Universal Link opens but no ApprovalSheet" — ROOT-CAUSED + FIXED 2026-09-02** (NOT a delivery bug). Sim instrumentation proved delivery works (`[Deeplink] received` fires, router returns the right outcome, `pendingNostrconnectURI` + `deeplinkApprovalURI` both set) but UIKit silently drops the `.sheet(item:)` presentation when the binding is set during another presentation animating in (the cold-launch onboarding→MainTabView root swap, or the launch-time notif-permission alert). Fix (in the stash-and-replay commit): defer the sheet-item set past the transition (`asyncAfter +0.6s`) + drain `pendingNostrconnectURI` in `onAppear` as well as `onChange`. Covers BOTH the new-user replay and the pre-existing existing-user Universal Link flow (same code path). AASA hypothesis discarded. Remaining: confirm on a physical phone (real `https` UL + APNs).
