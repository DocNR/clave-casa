...
- [x] **web: `static/brand/` "Sign in with Clave" button — DONE 2026-09-02** (`7bc368d`, signed off by Daniel; the neutral `--light`/`--dark` variants are the recommended default, `--brand` for a page's one primary action). OAuth-idiom button: 44px (override to 40px beside Google), 12px radius, current mark clipped to a 20px circle, 15px/600 system-stack label, site tokens throughout. Ships as a self-contained `sign-in-with-clave.css` (mark embedded), three 200×44 SVGs, and a partner README. Still to do in this area: wire the click via the rescoped `clave-connect.js`; update `docs/integrations.md` §Brand to point at `static/brand/` instead of "there isn't an official button yet".
